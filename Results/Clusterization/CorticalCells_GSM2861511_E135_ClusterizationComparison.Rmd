...
---
  title: "Compare the various clusterizations of Cortical Cells GSM2861511 E13.5"
---
...
setLoggingLevel(2)
setLoggingFile(file.path(".", "CompareClusterizations_CorticalCells_GSM2861511_E135.log"))
```
...
```{r}
cc135Obj <- readRDS(file = file.path(".", "CorticalCells_GSM2861511_E135.cotan.RDS"))

sampleCondition <- getMetadataElement(cc135Obj, datasetTags()[["cond"]])

logThis(sampleCondition)

allClust <- getClusterizations(cc135Obj)

logThis("")
logThis(paste("Number of cells:", getNumCells(cc135Obj)))

logThis("")
logThis("Available clusterizations:")
logThis(paste0(allClust, collapse = ", "))

clSizes <- vapply(allClust, function(x) { nlevels(getClusters(cc135Obj, x))}, integer(1L))
clSizes
...
```{r}
probMatrixDF <- read.csv(file.path(".", "Labeling", "E13_5_Devel_Mouse_Brain_probability_matrix.csv"), header = TRUE)
...
cells_to_keep <- rownames(probMatrixDF)[rownames(probMatrixDF) %in% getCells(cc135Obj)]
assert_that(identical(cells_to_keep, getCells(cc135Obj)))
```
...
```{r eval=FALSE}
set.seed(155)
shuffledCells <- sample(getCells(cc135Obj))

rndDist <- function(n) {
  clusters <- set_names(rep(1:n, ceiling(3000/n))[1:getNumCells(cc135Obj)], shuffledCells)
...
avgDists <- vapply(allClust, function(clName) {
  averageClustersDistance(normMatrix, getClusters(cc135Obj, clName))
}, numeric(1))
...
```{r}
tableAndPlot <- createSankey(cc135Obj, "cell.typist", "merge", space = 2, size = 2)
...
```{r}
cellTypistScores <- allMutualScores(cc135Obj, clName = "cell.typist")
...
write.csv(comparativeScores, file = "./CorticalCells_GSM2861511_E135_ClustersComparativeScores.csv")
```
