...
---
  title: "Compare the various clusterizations of Cortical Cells GSM2861514 E17.5"
---
...
setLoggingLevel(2)
setLoggingFile(file.path(".", "CompareClusterizations_CorticalCells_GSM2861514_E175.log"))
```
...
```{r}
cc175Obj <- readRDS(file = file.path(".", "CorticalCells_GSM2861514_E175.cotan.RDS"))

sampleCondition <- getMetadataElement(cc175Obj, datasetTags()[["cond"]])

logThis(sampleCondition)

allClust <- getClusterizations(cc175Obj)

logThis("")
logThis(paste("Number of cells:", getNumCells(cc175Obj)))

logThis("")
logThis("Available clusterizations:")
logThis(paste0(allClust, collapse = ", "))

clSizes <- vapply(allClust, function(x) { nlevels(getClusters(cc175Obj, x))}, integer(1L))
clSizes
...
```{r}
probMatrixDF <- read.csv(file.path(".", "Labeling", "E17_5_Devel_Mouse_Brain_probability_matrix.csv"), header = TRUE)
...
cells_to_keep <- rownames(probMatrixDF)[rownames(probMatrixDF) %in% getCells(cc175Obj)]
assert_that(identical(cells_to_keep, getCells(cc175Obj)))
```
...
```{r eval=FALSE}
set.seed(155)
shuffledCells <- sample(getCells(cc175Obj))

rndDist <- function(n) {
  clusters <- set_names(rep(1:n, ceiling(3000/n))[1:getNumCells(cc175Obj)], shuffledCells)
...
avgDists <- vapply(allClust, function(clName) {
  averageClustersDistance(normMatrix, getClusters(cc175Obj, clName))
}, numeric(1))
...
```{r}
tableAndPlot <- createSankey(cc175Obj, "cell.typist", "merge", space = 2, size = 2)
...
```{r}
cellTypistScores <- allMutualScores(cc175Obj, clName = "cell.typist")
...
write.csv(comparativeScores, file = "./CorticalCells_GSM2861514_E175_ClustersComparativeScores.csv")
```
