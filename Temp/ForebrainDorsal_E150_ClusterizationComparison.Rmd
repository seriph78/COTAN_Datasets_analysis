...
setLoggingLevel(2)
setLoggingFile(file.path(".", "CompareClusterizations_ForebrainDorsal_E150.log"))
```
...
```{r}
fb150Obj <- readRDS(file = file.path(".", "e15.0_ForebrainDorsal.cotan.RDS"))
...
```{r}
probMatrixDF <- read.csv(file.path(".", "Labeling", "E150_Devel_Mouse_Brain_probability_matrix.csv"), header = TRUE)
...
cells_to_keep <- rownames(probMatrixDF)[rownames(probMatrixDF) %in% getCells(fb150Obj)]
assert_that(identical(cells_to_keep, getCells(fb150Obj)))
```
...
```{r}
set.seed(155)
shuffledCells <- sample(getCells(fb150Obj))

rndDist <- function(n) {
  clusters <- set_names(rep(1:n, ceiling(9000/n))[1:getNumCells(fb150Obj)], shuffledCells)
...
avgDists <- vapply(allClust, function(clName) {
  averageClustersDistance(normMatrix, getClusters(fb150Obj, clName))
}, numeric(1))
...
```{r}
tableAndPlot <- createSankey(fb150Obj, "original.subclass", "merge", "seurat",
...
silhs <- vapply(allClust, function(clName) {
  clusters <- getClusters(fb150Obj, clName)
...
```{r}
cellTypistScores <- allMutualScores(fb150Obj, clName = "cell.typist")

cellTypistScores
```

```{r}
originalSubclassScores <- allMutualScores(fb150Obj, clName = "original.subclass")
...
```{r}
originalClusterScores <- allMutualScores(fb150Obj, clName = "original.clusters")
...
write.csv(comparativeScores, file = "./ForebrainDorsal_E150_ClustersComparativeScores.csv")
```
