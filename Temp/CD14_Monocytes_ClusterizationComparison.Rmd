---
  title: "Compare the various clusterizations of CD14 Monocytes"
---
...
setLoggingLevel(2)
setLoggingFile(file.path(".", "CompareClusterizations_CD14_Monocytes.log"))
```

This function creates a sankey plot, given a COTAN object and the names of 2 (or 3) clusterizations stored in the cells' metadata
...
```{r}
cd14Obj <- readRDS(file = file.path(".", "CD14_Monocytes.cotan.RDS"))

sampleCondition <- getMetadataElement(cd14Obj, datasetTags()[["cond"]])

logThis(sampleCondition)

allClust <- getClusterizations(cd14Obj)

logThis("")
logThis(paste("Number of cells:", getNumCells(cd14Obj)))

logThis("")
logThis("Available clusterizations:")
logThis(paste0(allClust, collapse = ", "))

clSizes <- vapply(allClust, function(x) { nlevels(getClusters(cd14Obj, x))}, integer(1L))
clSizes
...
```{r}
probMatrixDF <- read.csv(file.path(".", "Labeling", "CD14Cleaned_Immune_All_Low_probability_matrix.csv"), header = TRUE)
probMatrixDF <- column_to_rownames(probMatrixDF, var = "X")
rownames(probMatrixDF) <- gsub("[.]", "-", rownames(probMatrixDF))

cells_to_keep <- rownames(probMatrixDF)[rownames(probMatrixDF) %in% getCells(cd14Obj)]
...
```{r eval=FALSE}
set.seed(155)
shuffledCells <- sample(getCells(cd14Obj))

rndDist <- function(n) {
  clusters <- set_names(rep(1:n, ceiling(3000/n))[1:getNumCells(cd14Obj)], shuffledCells)
  d <- averageClustersDistance(normMatrix, clusters)
  return(d)
}

sizes <- c(1:15, seq(20, 50, by = 5), seq(60, 100, by = 10),
           seq(150, 1000, by = 50), 1500, 2000)
...
avgDists <- vapply(allClust, function(clName) {
  averageClustersDistance(normMatrix, getClusters(cd14Obj, clName))
}, numeric(1))
...
```{r}
tableAndPlot <- createSankey(cd14Obj,  "cell.typist", "merge", "seurat",
...
silhs <- vapply(allClust, function(clName) {
  clusters <- getClusters(cd14Obj, clName)
...
```{r}
cellTypistScores <- allMutualScores(cd14Obj, clName = "cell.typist")
...
write.csv(comparativeScores, file = "./CD14_Monocytes_ClustersComparativeScores.csv")
```
