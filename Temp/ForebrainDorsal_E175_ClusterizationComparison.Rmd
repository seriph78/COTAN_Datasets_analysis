...
---
  title: "Compare the various clusterizations of Forebrain Dorsal E17.5"
---
...
setLoggingLevel(2)
setLoggingFile(file.path(".", "CompareClusterizations_ForebrainDorsal_E175.log"))
```
...
```{r}
fb175Obj <- readRDS(file = file.path(".", "e17.5_ForebrainDorsal.cotan.RDS"))
...
```{r}
probMatrixDF <- read.csv(file.path(".", "Labeling", "E175_Devel_Mouse_Brain_probability_matrix.csv"), header = TRUE)
...
cells_to_keep <- rownames(probMatrixDF)[rownames(probMatrixDF) %in% getCells(fb175Obj)]
assert_that(identical(cells_to_keep, getCells(fb175Obj)))
```
...
```{r eval=FALSE}
set.seed(155)
shuffledCells <- sample(getCells(fb175Obj))

rndDist <- function(n) {
  clusters <- set_names(rep(1:n, ceiling(3000/n))[1:getNumCells(fb175Obj)], shuffledCells)
  d <- averageClustersDistance(normMatrix, clusters)
  return(d)
}

sizes <- c(1:15, seq(20, 50, by = 5), seq(60, 100, by = 10),
           seq(150, 1000, by = 50), seq(1500, 2500, by = 500))
...
avgDists <- vapply(allClust, function(clName) {
  averageClustersDistance(normMatrix, getClusters(fb175Obj, clName))
}, numeric(1))
...
```{r}
tableAndPlot <- createSankey(fb175Obj, "original.subclass", "merge", "seurat",
...
silhs <- vapply(allClust, function(clName) {
  clusters <- getClusters(fb175Obj, clName)
...
```{r}
cellTypistScores <- allMutualScores(fb175Obj, clName = "cell.typist")

cellTypistScores
```

```{r}
originalSubclassScores <- allMutualScores(fb175Obj, clName = "original.subclass")
...
```{r}
originalClusterScores <- allMutualScores(fb175Obj, clName = "original.clusters")
...
write.csv(comparativeScores, file = "./ForebrainDorsal_E175_ClustersComparativeScores.csv")
```
